import { Destroyable, DestroyLike } from "../Destroy";
import { EventbusDefine } from "../eventbus";
...
export interface ScrollableEventMap{
    onScroll:ScrollEvent
}

export class Scrollable extends Destroyable {

	_scrollableBrand: void;

	private _smoothScrollDuration: number;
	private readonly _scheduleAtNextAnimationFrame: (callback: () => void) => DestroyLike;
	private _state: ScrollState;
	private _smoothScrolling: SmoothScrollingOperation | null;

	public readonly emitter=new EventbusDefine<ScrollableEventMap>();

	constructor(smoothScrollDuration: number, scheduleAtNextAnimationFrame: (callback: () => void) => DestroyLike) {
		super();
	...
	private _setState(newState: ScrollState, inSmoothScrolling: boolean): void {
		...
		this.emitter.emit("onScroll",this._state.createScrollEvent(oldState, inSmoothScrolling));
	}
}
...
export class SmoothScrollingOperation {
	...
	public animationFrameDisposable: DestroyLike | null;
	...
	public destroy(): void {
		if (this.animationFrameDisposable !== null) {
			this.animationFrameDisposable.destroy();
			this.animationFrameDisposable = null;
		}
	}
